...
export type RegularSeasonHardestSchedule = {
  avatarUrl?: string;
  displayName: string;
...
export const getHardestRegularSeasonSchedule = cache(
  async ({ leagueId }: { leagueId: string }) => {
    ...
    pointsAgainstByRosterId.forEach((pointsAgainst, rosterId) => {
      ...
      if (pointsAgainst > topPointsAgainst) {
        topPointsAgainst = pointsAgainst;
        topRosterId = rosterId;
      }
    });

    if (!topRosterId || !Number.isFinite(topPointsAgainst)) {
      return null;
    }
    ...
    return {
      ...
      pointsAgainst: Math.round(topPointsAgainst * 100) / 100,
      teamName: topUser?.metadata?.team_name,
      userId: topUserId,
    } satisfies RegularSeasonHardestSchedule;
  },
);
